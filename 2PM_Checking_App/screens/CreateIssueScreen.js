...
export default function CreateIssueScreen({ navigation, route }) {
  ...
  const [siteId] = useState(route?.params?.siteId || null);
  const [siteName] = useState(route?.params?.siteName || "Site");
  const [linkedNotificationId] = useState(route?.params?.linkedNotificationId || null);
  ...
  async function handleSave() {
    ...
    try{
      ...
      // Link issue back to the check-in alert notification if this came from "Not ready"
